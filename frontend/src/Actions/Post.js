...
export const addCommentOnPost = (id, comment) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.put(`/api/v1/post/comment/${id}`, { comment });
...
export const createNewPost = (caption, image) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.post(`/api/v1/post/upload`, { caption, image });
...
export const updatePost = (caption, id) => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.put(`/api/v1/post/${id}`, { caption });
